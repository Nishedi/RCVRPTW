...
This paper presented a comprehensive study of the Capacitated Vehicle Routing Problem with Time Windows (CVRPTW) under stochastic conditions. The formulated mathematical model includes vehicle capacity constraints, time windows, service times, and operational cost components including travel distance, waiting time, and penalty costs for time window violations. Although the proposed approach does not constitute a formal robust optimization method in the theoretical sense, it exhibits characteristics of \emph{operational robustness} --- demonstrating stable performance across diverse stochastic scenarios.
...
\begin{itemize}
    ...
    \item \textbf{Enhanced Operational Robustness (H2):} The metaheuristic exhibited lower variability across scenarios (standard deviation: 5838.98 vs. 6266.17) and tighter distributional characteristics, as evidenced by consistently smaller interquartile ranges and upper percentiles. ECDF analysis confirmed this operational robustness across all Solomon instance families (clustered, random, and mixed).
...
\begin{enumerate}
    ...
    \item Empirical validation demonstrating that metaheuristic approaches (specifically Tabu Search) provide not only better average performance but also superior operational robustness and risk mitigation compared to constructive greedy methods.
