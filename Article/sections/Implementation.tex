...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Neighborhood generation using the specified mutation operator (swap, insert, invert, 2opt, oropt, or cross)
    ...
    \item \textbf{NeighborhoodGenerator:} Provides methods for generating neighboring solutions through local search operators. The \texttt{GenerateAllSwaps} method supports six mutation types:
    \begin{itemize}
        ...
        \item \textit{2opt:} Classic 2-opt operation that removes two edges and reconnects the path by reversing a segment between two positions
        \item \textit{oropt:} Removes a sequence of consecutive customers (length 1-3) and reinserts them elsewhere in the route
        \item \textit{cross:} Swaps two segments of potentially different lengths from different parts of the route(s)
    \end{itemize}
