...
The implementation follows object-oriented design principles and consists of several core components organized into a modular architecture. Figure~\ref{fig:architecture} conceptually illustrates the main modules and their interactions.
...
\begin{itemize}
    ...
    \item \textbf{Lazy Neighborhood Generation:} The Tabu Search implementation generates neighbors on-demand using LINQ's lazy evaluation (\texttt{.Take(TabuSize*10)}), terminating generation once sufficient candidates are found.
...
\begin{itemize}
    \item Explicit random seed initialization (\texttt{baseSeed = 42})
