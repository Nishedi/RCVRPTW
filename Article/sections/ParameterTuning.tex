...
\begin{itemize}
    \item \textbf{Neighborhood Type}: swap, insert, invert.
...
Table~\ref{tab:mutation_summary} provides statistics for each mutation type. The \texttt{swap} operator not only achieves the lowest mean objective (11449.25) but also demonstrates the smallest variation, indicating more consistent performance.
...
Table~\ref{tab:best_configs} summarizes the best parameter configurations identified for each mutation type. The overall best configuration uses the \texttt{swap} operator with tabu size of 50 and a no-improvement limit of 100 iterations, achieving a mean objective value of 11263.33.
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|c|c|c}
        \hline\hline
        Mutation & Tabu Size & No-Improvement Limit & Mean Objective \\
        \hline\hline
...
\begin{enumerate}
    \item \textbf{Mutation operator}: The \texttt{swap} operator significantly outperforms \texttt{insert} and \texttt{invert}, providing 6--10\% lower objective values on average.
