...
\begin{abstract}
Modern logistics systems operate under uncertain and dynamically changing conditions, particularly in relation to customer demand and delivery time windows. This paper investigates a stochastic variant of the Capacitated Vehicle Routing Problem with Time Windows (CVRPTW), where both time-window boundaries and customer demands are perturbed across multiple scenarios. Although the approach does not follow a formal robust optimization framework, solution robustness is evaluated through large-scale scenario-based analysis. A complete MILP model integrating vehicle capacity constraints, soft time windows and penalty-based feasibility relaxation is formulated. To solve large stochastic instances, a Tabu Search metaheuristic is developed and compared with a greedy constructive baseline.

The experimental evaluation over 3,000 stochastic scenarios derived from Solomon benchmarks demonstrates that Tabu Search consistently outperforms Greedy — achieving lower average cost, reduced variability, smaller time-window penalties, and fewer extreme high-cost outcomes. These results confirm that the metaheuristic provides robust routing performance under uncertainty, making it well-suited for real-world applications.
...
\begin{IEEEkeywords}
CVRPTW, robustness, stochastic, time windows, MILP, Tabu Search
\end{IEEEkeywords}
